...
1. **工具条里要放"模型切换"**（用户已定方向）。现在工具条里**只有发送 / 停止**，切换没做。
   做的时候不需要新接口 —— §8.14 的 `GET /api/llm/profiles` + `POST /api/llm/active` 就是它；
   Mock 模式下要显示"未接入"而不是空列表（对着假后端显示模型配置是骗人的）。
2. **高度没谈拢**。实测：`min-h` 从 `3rem` 调到 `2.5rem`，整块只矮了 **8px**
   （空态 122 → 114px、对话中 118 → 110px），用户的原话是"高度一点变化也没有"——
   这么改确实看不出来：整块高度里占大头的是**工具条那一行（icon 按钮 `h-10`）**和
   上下内边距（26px），只动 `min-height` 动不到观感。
   ⚠️ 收尾时用户说"算了"，所以这里**保持原样、没有继续改**。下次要动就从
   「按钮行高 / 内边距 / 把按钮并回文本域那一行」里挑一个，别再只动 `min-height`。

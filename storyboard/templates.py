"""
Manim 代码模板库 —— 旧架构的遗留层，**仅用于兼容 examples/ 下的老分镜**。

⚠️ 新分镜请用 storyboard/dsl.py 的元素+时间线写法，不要在这里加新模板：
   模板枚举会把组合空间锁死在「9 个模板 × 各自几个参数」，画面必然千篇一律。

注：本文件里的 SAFE_EXPR_HELPER 已不再被 renderer 注入（统一改用
    dsl.RUNTIME_HELPER，那里 _eval_expr 兼容两参调用且多了 NaN/Inf 兜底）。
    保留它只是为了单看本文件时上下文完整。

以下是模板层原本的三条设计原则（DSL 层全部继承，只是不再靠"模板"实现）：
